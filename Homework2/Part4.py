...
weigh = []
get_w = keras.callbacks.LambdaCallback(on_batch_begin= lambda epoch, logs: weigh.append(models.get_weights()) )

models = model()
history = models.fit(x = X, y = Y, batch_size= 1, epochs = 1, validation_split= 0 , shuffle= False, callbacks= [get_w])
#Save model

np.save("Weights_Hand.npy" , weigh)
